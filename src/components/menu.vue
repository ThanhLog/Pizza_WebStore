<template>
    <div class="main shadow-lg sticky top-0 left-0">
        <div class=" relative">
            <div class="Menu w-full h-full p-3">
                <div class="logo">
                    <router-link to="/">
                        <img src="../assets/logo.png" alt="logo" style="height: 40px">
                    </router-link>
                </div>
                <div class="itemMenu h-full">
                    <button class="Notify border-none" @click="togglePages('notify')">
                    ...
                    <div class=" account ">
                        <button class="Login flex" @click="togglePages('login')">
                            <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
                                <path fill="#CB2843"
                        ...
                        <button class="TrackOder flex" @click="togglePages('trackOrder')">
                            <svg xmlns="http://www.w3.org/2000/svg" width="2rem" height="2rem" viewBox="0 0 56 34">
                                <defs>
                                    <filter id="a" width="102.9%" height="112.5%" x="-1.5%" y="-4.5%"
                                ...
                                <g fill="none" fill-rule="evenodd" filter="url(#a)" transform="translate(1.51 1)">
                                    ...
                                    <ellipse cx="36.8" cy="15.2" stroke="#C8102E" stroke-width="4" rx="10.4"
                                        ry="11.2" />
                                    <path fill="#C8102E"
                                        d="M44.623 22.401l2.848-.801 4.164 3.616c.593 1.527.464 2.756-.387 3.686-.85.93-2.11.93-3.777 0L44 25.216l.623-2.815z" />
                                    <path stroke="#FFF" stroke-width=".8"
                                        d="M15.2 15.335l1.6-.135M19.2 9.6l1.6.136M20.8 12.831c1.039.997 1.042 1.064.147-.031" />
                                </g>
                            </svg>
                            <span class="text-xl">Track Oder!</span>
                        </button>
                    </div>
                </div>
            </div>
            <div class="h-full w-80 absolute right-[0]" style="bottom: -100%" v-if="mainPage">
                <div class="notify bg-white h-screen bg-stone-200 z-10" v-if="currentPage === 'notify'">
                    <div class=" p-1" style=" background: #282828">
                ...
                <div class="login shadow-lg shadow-[#111]-500/40 bg-white p-3 z-50" v-if="currentPage === 'login'">
                    <div class=" flex items-center justify-center gap-5 p-3 border-b-2 ">
                        <img src="../assets/favicon.ico" alt="" width="50px" height="50px">
                        <h1 class="text-4xl font-bold">Sign In</h1>
                    </div>
                    <div class=" mt-5 px-4 h-full flex flex-col items-center  gap-1">
                        <form action="username" class="flex w-full border-2 rounded-md">
                            <div class=" border-r-2 p-1">
                                <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
                                    <circle cx="12" cy="6" r="4" fill="#282828" />
                                    <path fill="#282828"
                        ...
                        <form action="password" class="flex w-full border-2 rounded-md ">
                            <div class=" border-r-2 p-1">
                                <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
                            ...
                            <div class="w-full flex items-center px-1">
                                <input :type="show === true ? 'text' : 'password'" v-model="password"
                                    placeholder="Password" class=" w-full outline-none p-2">
                            </div>
                        </form>
                    </div>
                    <div class="w-full">
                        <div class="mt-[10px]">
                            <a href="" class=" min-w-full text-end hover:text-cyan-500 ">Forgot Password?</a>
                        </div>
                        <div class=" w-full mt-[10px] px-2">
                            <button class=" bg-[#0A8020] w-full p-2 text-white font-bold rounded-[25px]">Sign
                                In</button>
                        </div>
                        <div>
                            <p class=" text-[13px] mt-3">You don't have an account?
                                <a href="" class=" text-[#0A8020] hover:underline hover:decoration-1">Register now</a>
                                or learn more about
                                <a href="" class=" text-[#0A8020] hover:underline hover:decoration-1">Membership Terms
                                    and
                                    Benefits.</a>
                            </p>
                        </div>
                ...
                <div class="trackOder bg-white shadow-lg shadow-[#111]-500/40 p-4" v-if="currentPage === 'trackOrder'">
                    <div class=" text-center">
                        <h3 class=" font-bold">TRACK YOUR ORDER</h3>
                        <p>Only available for Delivery order.</p>
                    </div>
                    <div class=" mt-6">
                        <input type="tel" class="w-full border-2 p-2 rounded-md outline-none"
                            placeholder="Phone number or order code*">
                        <button class=" mt-2 px-5 bg-[#0A8020] text-white w-full p-1 rounded-md">SEARCH</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        togglePages(page) {
            if (this.currentPage === page) {
                this.mainPage = null;
                this.currentPage = null; // Nếu trang hiện tại đã được chọn thì đóng nó
            } else {
                this.mainPage = page;   
                this.currentPage = page; // Nếu không, chuyển đến trang mới
                console.log(page)
            }
        }
    },
...
.notify {
    animation: sildeNotify 0.25s ease-in-out;
}

.login,
.trackOder {
    animation: slideShow 0.25s ease-in-out;
}

@keyframes sildeNotify {
    0% {
